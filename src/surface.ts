...
export interface Abs { readonly tag: 'Abs'; readonly name: Name; readonly type: Term | null; readonly body: Term }
...
export interface Let { readonly tag: 'Let'; readonly name: Name; readonly type: Term | null; readonly val: Term; readonly body: Term }
...
export const show = (t: Term): string => {
  ...
  if (t.tag === 'Abs') {
    ...
    return `\\${params.map(([x, t]) => t ? `(${x} : ${show(t)})` : x).join(' ')}. ${show(body)}`;
  }
  if (t.tag === 'App') {
  ...
  if (t.tag === 'Let')
    return `let ${t.name}${t.type ? ` : ${showP(t.type.tag === 'Let', t.type)}` : ''} = ${showP(t.val.tag === 'Let', t.val)}; ${show(t.body)}`;
  return t;
};
...
export const fromCore = (t: C.Term, ns: List<Name> = Nil): Term => {
  ...
  if (t.tag === 'Pi') {
    ...
    return Pi(x, fromCore(t.type, ns), fromCore(t.body, Cons(x, ns)));
  }
  if (t.tag === 'Abs') {
    const x = chooseName(t.name, ns);
    return Abs(x, fromCore(t.type, ns), fromCore(t.body, Cons(x, ns)));
  }
  if (t.tag === 'Let') {
    const x = chooseName(t.name, ns);
    return Let(x, fromCore(t.type, ns), fromCore(t.val, ns), fromCore(t.body, Cons(x, ns)));
  }
  return t;
};
