...
export interface VNe { readonly tag: 'VNe'; head: Head; spine: Spine }
export const VNe = (head: Head, spine: Spine): VNe => ({ tag: 'VNe', head, spine });
export interface VAbs { readonly tag: 'VAbs'; name: Name, type: Val, clos: Clos }
export const VAbs = (name: Name, type: Val, clos: Clos): VAbs => ({ tag: 'VAbs', name, type, clos });
export interface VPi { readonly tag: 'VPi'; name: Name, type: Val, clos: Clos }
...
export const evaluate = (t: Term, vs: EnvV): Val => {
  ...
  if (t.tag === 'Abs')
    return VAbs(t.name, evaluate(t.type, vs), v => evaluate(t.body, Cons(v, vs)));
  if (t.tag === 'Pi')
    return VPi(t.name, evaluate(t.type, vs), v => evaluate(t.body, Cons(v, vs)));
  if (t.tag === 'Var') 
